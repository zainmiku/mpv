...
local o = {
    ...
    font_mono = "Sarasa Term SC Nerd",   -- monospaced digits are sufficient
    font_size = 8,
    font_color = "FFFFFF",
    border_size = 0.8,
    border_color = "262626",
...
local function add_file(s)
    ...
    if demuxer_cache + demuxer_secs > 0 then
        append(s, utils.format_bytes_humanized(demuxer_cache), {prefix="缓冲总计："})
...
local function cache_stats()
    ...
    if seek_ts ~= nil then
        state = "跳转中（至 " .. mp.format_time(seek_ts) .. "）"
    ...
    append(stats, fc, {prefix = "本地文件缓冲："})

    append(stats, info["debug-low-level-seeks"], {prefix = "媒体跳转计数："})
    append(stats, info["debug-byte-level-seeks"], {prefix = "流的跳转计数："})

    append(stats, "", {prefix=o.nl .. o.nl .. "范围：", nl="", indent=""})
